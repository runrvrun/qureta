@extends('layouts.profile') @section('addcss')
<link rel="stylesheet" type="text/css" href="/slick/slick.css" />
<link rel="stylesheet" type="text/css" href="/slick/slick-theme.css" /> @endsection @section('content')
<?php Carbon::setLocale('id') ?> @if(Auth::Check())
<input type="hidden" id="followerid" value="{{ Auth::user()->id }}" /> @endif
<div id="wrap" class="desktop-only">
    <div class="profile-new-desktop" style="background-color:#efefef;display:block;padding-bottom:2em;min-height:60em;">
        <div class="header-profile-new" style="background:linear-gradient(rgba(0,0,0,0)0%,rgba(0,0,0,0)60%,rgba(0,0,0,0.5)85%,rgba(0,0,0,0.9)100%),url( @if(empty($users->user_image_cover)) http://bogor.net/wp-content/uploads/2018/02/hand_writing_book_man_7986_1920x1080.jpg @else {{ '/uploads/cover/'.$users->user_image_cover }} @endif);background-size: cover;">
            <div class="profile-new-atas">
                <div class="user-avatar" style="float:left;">
                    ...
                    <img src="{{ $users->user_image }}" onerror="avaError(this);"> @else
                    <img src="{{ URL::asset('/uploads/avatar/'.$users->user_image) }}" onerror="avaError(this);"> @endif
                </div>
            ...
            <div class="user-info">
                <div class="user-info-top row" style="margin:0px;">
                    <div class="col-md-6">
                        <i class="fa fa-file-text-o" style="float:left;padding:3px;" aria-hidden="true"></i>
                        <span style="float:right">{{ $jml_post }}</span>
                        <div class="clearfix"></div>
                    </div>
                    <div class="col-md-6">
                        <i class="fa fa-book" style="float:left;padding:3px;" aria-hidden="true"></i>
                        <span style="float:right">{{ $jml_buqu }}</span>
                    </div>
                </div>
                ...
                <div class="user-info-bottom row" style="margin:0px;">
                    ...
                    <div class="col-md-6">
                        <center>
                            <a href="#myModal2" data-toggle="modal" style="color:#FFF">
            ...
            <div class="row">
                ...
                @endif
                 @foreach ($posts as $key=>$row)
                    ...
                    @endforeach @if($jml_post>count($posts))
                    <center>
                        <a href="/profile/tulisan/{{ $users->username }}">Tulisan {{ $users->name }} Lainnya...</a>
                    </center>
                    @endif @endif
                </div>
                <div class="col-md-3">
                        <h3>Penulis lainnya</h3>
                    <?php $recommended_writers = get_recommended_user(); ?> @if(count($recommended_writers)>0) @foreach($recommended_writers as $recw)
                    <a href="'/profile/'{{ $recw->user->username}}">
                        <div class="row recommended_writers_box" style="height:6em;min-height:6em;background-color:white;margin:10px 0px;border-radius:3px;">
                                ...
                                <center>
                                    <a href="/profile/{{ $recw->user->username }}">
                                        <div class="image" style="background-color:#ddd;height:4em;width:4em;border-radius:100%;overflow:hidden;margin:15px 0px;">
                                            ...
                                            <img style="height:100%;max-height:100%;width:auto" src="{{ $recw->user->user_image }}" alt="{{ $recw->user->user_image }}"
                                                onerror="avaError(this);" /> @else
                                            <img style="height:100%;max-height:100%;width:auto" src="{{ URL::asset('/uploads/avatar/'.$recw->user->user_image) }}" alt="{{ $recw->user->user_image }}"
                                                onerror="avaError(this);" /> @endif
                                        </div>
                                    </a>
                                </center>
                            </div>
                            <div class="col-md-8" style="padding:4px 6px;">
                                ...
                                @if(isset($recw->user->role) && ($recw->user->role == 'premium' || $recw->user->role == 'partner' || $recw->user->role ==
                                'admin' || $recw->user->role == 'editor'))
                                <i class="verified-user" style="font-size:15px;width:20px !important;"></i>
                                @endif
                                <i style="display:block;text-align:left;margin-right:10px;opacity:.6;">{{ get_user_profesi($recw->user->id) }}</i>
                                <div style="margin-top:10px;"></div>
                                <!-- <center>
...
<div id="wrap" class="mobile-only">
    ...
    <div class="header-profile-new-mobile" style="height:13em;background:linear-gradient(rgba(0,0,0,0)0%,rgba(0,0,0,0)60%,rgba(0,0,0,0.5)85%,rgba(0,0,0,0.9)100%),url( @if(empty($users->user_image_cover)) http://bogor.net/wp-content/uploads/2018/02/hand_writing_book_man_7986_1920x1080.jpg @else {{ '/uploads/cover/'.$users->user_image_cover }} @endif);background-size: cover;background-position:right top;">
        <div class="clearfix"></div>
    </div>
    <div class="" style="min-height:25em;background-color:#efefef;">
        <!-- <div style="margin-top:5em;"></div> -->
        <div style="position: relative;top:-2.85em;">
            <div class="box-mobile-profile" style="margin:0px;padding:0px;width:75%;margin:auto;background-color:rgba(0,0,0,0.7);min-height:10em;box-shadow:1px 1px 4px rgba(0,0,0,0.7)">
                <div class="row" style="margin:0px;padding:10px;border-bottom:1px solid white;margin-bottom:5px;background-color:rgba(0,0,0,0.2);">
                    <div class="col-xs-4">
                    <a style="color:white" href="/profile/tulisan/{{ $users->username }}">
                            <i style="float:left" class="fa fa-file-text-o" style="padding:3px;" aria-hidden="true"></i>
                            <span style="float:right">{{ $jml_post }}</span>
                        </a>
                        <div class="clearfix"></div>
                    </div>
                    ...
                    <a style="color:white" href="/profile/buqu/{{ $users->username }}">
                        <i class="fa fa-book" style="float:left;padding:3px;" aria-hidden="true"></i>
                        <span style="float:right">{{ $jml_buqu }}</span>
                        </a>
                    </div>
                ...
                <div style="padding:10px;">
                    ...
                    <div style="float:left;margin-left:10px;">
                        <div class="user-info">
                            <h4 style="display:inline-block;padding:0px;margin:0px;">{{ $users->name }}</h4>
                            @if(isset($users->role) && ($users->role == 'premium' || $users->role == 'admin' || $users->role == 'editor'))
                            <small class="verified-user" style="width:15px">&nbsp;</small>
                            @endif
                            <br>
                    ...
                    <div class="clearfix"></div>
                    <div style="margin:0px 25px;padding-left:5px;color:white;margin-top:10px;max-height:13em;overflow-y:scroll;">
                        @if(!empty($profile['short_bio']))
                        {{ $profile['short_bio'] }}
                        @endif
                    </div>
            ...
            <div style="background-color:white;width:95%;margin:auto;border-radius:10px;">
                ...
                @foreach ($posts as $key=>$row)
                    <a href="{{ url('post/'.$row->post_slug) }}">
                    <div class="row" style="margin:0px;padding:0px;height:6em;">
                        <div class="col-xs-5" style="margin:0px;padding:0px;overflow:hidden;">
                            <img src="{{ URL::asset('/uploads/post/thumb/'.$row->post_image) }}" alt="{{ $row->post_image }}" onerror="imgError(this);" style="width:100%;height:auto;">
                        </div>
                        <div class="col-xs-7" style="margin:0px;padding:3px;">
                            <div style="font-size:10px;color:black;opacity:.7;">
                                <div class="col-xs-6">
                                    <i class="fa fa-eye"></i> {{ number_format($row->view_count,0,',','.') }} views
                                </div>
                                <div class="col-xs-6">
                            ...
                            <div style="display:block;"> {{$row->post_title}} </div>
                        </div>
                    </div>
                    </a>
                    <div class="clearfix"></div>
            ...
            @endif
            @if(count($buqus) > 0)
            <div style="background-color:white;padding:5px 0px;margin-top:1em;">
            <div class="" style="width:80%;margin:auto;height:20em;">
                <div class="buqu-slick" style="display:block;overflow:hidden;border-radius:5px;"data-slick='{"slidesToShow": 2, "slidesToScroll": 1}'>
                @foreach ($buqus as $row)
                    <a href="{{ url('/buqu/'.$row->buqu_slug) }}" style="display:inline-block;position:relative;">
                        <center>
                        <img style="width:100%;max-width:100%;" src="{{ URL::asset('uploads/buqu/'.$row->buqu_image) }}" onerror="buquError(this);"/>
                ...
                <p style="margin-top:10px;" align="center">{{ HTML::link('/profile/buqu/'.$users->username,'Lihat semua buqu') }}</p>
            </div>
            </div>
            @endif
            ...
            <?php $recommended_writers = get_recommended_user(); ?> @if(count($recommended_writers)>0) @foreach($recommended_writers as $recw)
                    <a href="'/profile/'{{ $recw->user->username}}">
                        <div class="row recommended_writers_box" style="height:4em;min-height:4em;background-color:white;margin:10px 0px;border-radius:3px;">
                            <div class="col-md-4 col-xs-2" style="height:100%;">
                                <center>
                                    <a href="/profile/{{ $recw->user->username }}">
                                        <div class="image" style="background-color:#ddd;height:3em;width:3em;border-radius:100%;overflow:hidden;margin:5px 0px;">
                                            @if(strpos($recw->user_image,'ttps://') || strpos($recw->user_image,'ttp://'))
                                            <img style="height:100%;max-height:100%;width:auto" src="{{ $recw->user->user_image }}" alt="{{ $recw->user->user_image }}"
                                                onerror="avaError(this);" /> @else
                                            <img style="height:100%;max-height:100%;width:auto" src="{{ URL::asset('/uploads/avatar/'.$recw->user->user_image) }}" alt="{{ $recw->user->user_image }}"
                            ...
                            <div class="col-md-8 col-xs-10" style="padding:4px 6px;">
                                <h5 style="display: inline-block !important;margin:0px;">
                                    {{ HTML::link('/profile/'.$recw->user->username, $recw->user->name)}}
                                </h5>
                                @if(isset($recw->user->role) && ($recw->user->role == 'premium' || $recw->user->role == 'partner' || $recw->user->role ==
                ...
                <div class="tab-pane" id="pengikut">
                    ...
                    <div class="row">
                        ...
                        <div class="article col-sm-3 grid-group-item">
                            ...
                            <div class="user-info">
                                <div class="image">
                                    <img src="{{ URL::asset('/uploads/avatar/'.$row->followers->user_image) }}" onerror="avaError(this);" />
                                </div>
                ...
                <div class="tab-pane" id="mengikuti">
                    ...
                    <div class="row">
                        ...
                        <div class="article col-sm-3 grid-group-item">
                            ...
                            <div class="user-info">
                                <div class="image">
                                    <img src="{{ URL::asset('/uploads/avatar/'.$row->users->user_image) }}" onerror="avaError(this);" />
                                </div>
        ...
        $('.btnLike').click(function () {
            ...
            if ($this.hasClass('active')) {
                ...
                $.ajax({
                    ...
                    success: function () {
                        $('.like-counter' + postid).html(parseInt($('.like-counter' + postid).html(),
                            10) + 1)
                    }
                });
            } else {
            ...
            if (cookies_enabled()) {
                if ($.cookie("share" + postid)) {
                    ...
                    $.ajax({
                        ...
                        success: function () {
                            $.cookie("share" + postid, postid, {
                                expires: 1
                            });
                            $('.share-counter' + postid).html(parseInt($('.share-counter' + postid)
                                .html(), 10) + 1)
                        }
                    });
                }
        ...
        $('.btnLikeBuqu').click(function () {
            ...
            if ($this.hasClass('active')) {
                ...
                $.ajax({
                    ...
                    success: function () {
                        $('.like-counterbuqu' + postid).html(parseInt($('.like-counterbuqu' +
                            postid).html(), 10) + 1)
                    }
                });
            } else {
            ...
            if (cookies_enabled()) {
                if ($.cookie("sharebuqu" + postid)) {
                    ...
                    $.ajax({
                        ...
                        success: function () {
                            $.cookie("sharebuqu" + postid, postid, {
                                expires: 1
                            });
                            $('.share-counterbuqu' + postid).html(parseInt($('.share-counterbuqu' +
                                postid).html(), 10) + 1)
                        }
                    });
                }
